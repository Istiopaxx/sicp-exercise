...
(define (install-develop-division-package)
  ...
  (put 'insert-record 'develop insert-record)
  (put 'get-record 'develop get-record)
  (put 'make-record 'develop (lambda (name salary) (tag (make-record name salary))))
  (put 'get-salary 'develop get-salary)
  'done)
...
(define (install-design-division-package)
  ...
  (define (get-record name file)
    (define (iter x set)
      (let ((now-x (get-name (car set))))
        (cond ((null? set) #f)
              ((= x now-x) (car set))
              ((< x now-x) #f)
              (else (iter x (cdr set))))))
    (let ((result (iter name file)))
      (if (pair? result)
          result
          ; or.. can choose throwing (error "no name in this file -- get-record" name).
          #f)))
  ...
  (put 'insert-record 'design insert-record)
  (put 'get-record 'design get-record)
  (put 'make-record 'design (lambda (name salary) (tag (make-record name salary))))
  (put 'get-salary 'design get-salary)
  'done)
...
(define (iter-dev n result)
  (if (= n 0)
      ...
      (let ((make-record (get 'make-record 'develop))
        ...
        (iter-dev (- n 1)
                  (insert-record (contents (make-record random-name random-salary))
                                 result)))))
(define develop-division-file (attach-tag 'develop (iter-dev 30 nil)))
  ...
  (if (= n 0)
      ...
      (let ((make-record (get 'make-record 'design))
        ...
        (iter-des (- n 1)
                  (insert-record (contents (make-record random-name random-salary))
                                 result)))))
(define design-division-file (attach-tag 'design (iter-des 30 nil)))
...
(define (get-record employee-name file)
  (let ((division (type-tag file))
        (record-file (contents file)))
    (attach-tag division
                ((get 'get-record division) employee-name record-file))))
